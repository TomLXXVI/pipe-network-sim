...
from lib.pypeflow.core.cross_sections import Circular
...
def darcy_friction_factor(re: float, rel_pipe_rough: float) -> float:
    """
    Calculate the Darcy friction factor (with Serghide's  equation).
...
class Pipe:
    ...
    def reynolds_number(self) -> float:
        """Get the Reynolds number (*float*) for the flow in the pipe."""
        return reynolds_number(
            self.velocity('m/s'),
            self.cross_section.diameter('m'),
            self._fluid.kinematic_viscosity()
        )
