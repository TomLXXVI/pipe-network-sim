...
from lib.pypeflow.core.fluids import Fluid
from lib.pypeflow.core.pipe_schedules import PipeSchedule
...
class Node:
    ...
    def connect(self, section: 'Section', direction: str):
        ...
        """
        if direction == 'in':
            self._in.setdefault(section.id, section)  # only connect section if key section.id does not exist already
...
class Section:
    ...
    def __init__(self, section_id: str, loop_id: str, start_node: Node, end_node: Node):
        ...
        **Parameters:**
        ...
        self.start_node.connect(self, 'out')  # connect section as an outgoing section to its start node
        self.end_node.connect(self, 'in')  # connect section as an incoming section to its end node
        self.type: str = ''
        self.sign: int = 1
...
class FlowPath(List[Section]):
    ...
    def head_loss(self) -> qty.Pressure:
        ...
        sum_ = sum([section.sign * section.pressure_drop() for section in self if section.type != 'pseudo'])
...
class Network:
    ...
    def __init__(self):
        ...
        self.network_section: Optional[Section] = None
        # equivalent section that represents the hydraulic resistance of the network
    ...
    def add_section(self, **kwargs):
        ...
        **kwargs:**
        ...
        if start_node.id == self.start_node_id and end_node.id == self.end_node_id:
            self.network_section = section
        loop = self._loops.setdefault(loop_id, Loop(loop_id))
        loop.add_section(section)
        section_list = self.sections.setdefault(section_id, [])
        section_list.append(section)
        # if section with `section_id` is not in the dict, create key `section_id` that maps to an empty list and
        # return this empty list as `section_list`. Then add the section to this list.
        # if `section_id` is already present, return the list also as `section_list` and append the new section to this
        # list. This is done as the same section may belong to two different loops.
            ...
            for section in loop.sections.values():
                if section.type != 'pseudo':
                    ...
                    if len(section_list) == 2:
                        ...
                        corr_term = loop.corr_term - self._loops[second_loop_id].corr_term
                    else:
    ...
    def _check_loops(self, error: float):
        ...
        if False in [abs(loop.pressure_drop) < error for loop in self._loops.values()]:
            return False
        return True
    ...
    def _search(self, node: Node, path: FlowPath):
        ...
        # if the current node has more than 1 outgoing section, then
        # for each outgoing section except the first one...
        #   create a new empty path
        #   add the current path to the new path
        #   add the section to the new path
        #   add the new path to the list of paths in the network
        #   create a thread that begins searching from the end node of the section
        # add the first outgoing section of the current node to the current path
        # and then set current node to the end node of the first outgoing section
    ...
    def check_flow_balance(self, V_ext: Dict[str, Dict[str, Optional[List[qty.VolumeFlowRate]]]] = None) \
        ...
        for node in self.nodes.values():
            V_in = None; V_out = None
            if V_ext is not None:
                if node.id in V_ext:
                    V_dict = V_ext.get(node.id)
                    if V_dict['in'] is not None:
                        V_in = V_dict['in']
                    if V_dict['out'] is not None:
                        V_out = V_dict['out']
            dif = node.check_flow_balance(V_ext_in=V_in, V_ext_out=V_out)
            checks[node.id] = qty.VolumeFlowRate(dif)
        return checks

    def clear(self):
        """Clear network internal objects."""
        self._loops = {}
        self.nodes = {}
        self.sections = {}
        self._paths = []
