...
from lib.pypeflow.core.fluids import Fluid, FLUIDS
from lib.pypeflow.design.network import Network
...
class Designer:
    ...
    def configure_network(cls, file_path: str):
        """
        Configure network with a network configuration file (.csv-file). Parameter `file_path` (*str*) is the file path
            ...
            for i, row in enumerate(reader):
                if i == 0:
                    ...
                    cls.network.add_section(
                        id=row[0],
                        start_node_id=row[1],
                        start_node_height=qty.create_from_str(qty.Length, row[2], cls.units['length'], 0.0),
                        end_node_id=row[3],
                        end_node_height=qty.create_from_str(qty.Length, row[4], cls.units['length'], 0.0),
                        length=qty.create_from_str(qty.Length, row[5], cls.units['length'], 0.0),
                        nominal_diameter=qty.create_from_str(qty.Length, row[6], cls.units['diameter'], None),
                        flow_rate=qty.create_from_str(qty.VolumeFlowRate, row[7], cls.units['flow_rate'], None),
                        pressure_drop=qty.create_from_str(qty.Pressure, row[8], cls.units['pressure'], None),
                    )
    ...
    def get_sections(cls) -> pd.DataFrame:
        ...
        for section in cls.network.sections.values():
            ...
            d[keys[7]].append(section.pipe.reynolds_number)
            d[keys[8]].append(section.pressure_drop(cls.units['pressure'], 3))
